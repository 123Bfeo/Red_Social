...
export class AuthService {
  ...
  async create(userDto: UserDto) {
    try {
      const hashedPassword = await this.helpServices.hashPassword(
        userDto.password,
      );
      ...
      throw new InternalServerErrorException(
        `Error create auth: ${error.message} `,
      );
    }
  }
  ...
  async validationUserLogin(email: string, password: string) {
    ...
    try {
      const userEmail = await this.userEntityRepository.findOne({
        where: { email: email },
      });
      if (!userEmail) {
        throw new UnauthorizedException('Invalid Credentials');
      }
      const validatePass: boolean = await this.helpServices.comparePassword(
        pass,
        userEmail.password,
      );
      if (!validatePass) {
      ...
      throw new InternalServerErrorException(
        `Error not fount user: ${error.message} `,
      );
    }
  }
  async loginJWT(iduser: number, email: string) {
    try {
      ...
      throw new InternalServerErrorException(
        `Error not jwt user: ${error.message} `,
      );
    }
  }
  ...
  private entityToDto(user: UserEntity) {
    const { iduser, fullname, age, email, createdAt, updatedAt, deletedAt } =
      user;
